...
def predict(spectrogram, model, noisy_signal, alpha, beta, alpha_cum, sigmas, T,c1, c2, c3, delta, delta_bar, device=torch.device('cuda')):
  with torch.no_grad():
    ...
    spectrogram = spectrogram.to(device) # torch.Size([1, 20, 104])
    
    audio = torch.randn(spectrogram.shape[0], model.params.hop_samples * spectrogram.shape[-1], device=device)
    noise_scale = torch.from_numpy(alpha_cum**0.5).float().unsqueeze(1).to(device)
    noisy_audio = torch.zeros(spectrogram.shape[0], model.params.hop_samples * spectrogram.shape[-1], device=device) # torch.Size([1, 1664])
    noisy_audio[:,:noisy_signal.shape[0]] = torch.from_numpy(noisy_signal).to(device) # noisy_signal (1650,)
    audio = noisy_audio
    gamma = [0.2]
    for n in range(len(alpha) - 1, -1, -1):
      if n > 0:
...
def main(args):
  ...
  for spec in tqdm(specnames):
    ...
    noisy_signal, _ = librosa.load(os.path.join(args.wav_path,spec.split("/")[-1].replace(".spec.npy","")),sr=55)
    ...
    torchaudio.save(output_name, audio.cpu(), sample_rate=sr) # 保存去噪的音频
